...
async function dataValidation(username, password) {
    ...
    if (password && (password === password.split(' ').join('')) && password.length > 5) dataChecks.password = true
    else dataChecks.password = false;
    // check if username is valid
    if (username && (username === username.split(' ').join('')) && username.length < 50) dataChecks.username = true;
...
async function registerUser(data) {
    ...
    else if (dataCheck.password == false) return [401, "Invalid password. Must be at least 5 characters with no spaces"]
